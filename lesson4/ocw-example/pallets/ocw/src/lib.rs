...
pub mod pallet {
	...
	// 获取当前DOT价格
	const HTTP_REMOTE_DOT_PRICE_REQUEST: &str = "https://api.coincap.io/v2/assets/polkadot";
	// 携带用户头
	const HTTP_HEADER_USER_AGENT: &str = "lessichen";
	...
	pub enum Event<T: Config> {
		...
		NewPrice(Option<T::AccountId>, u64, u32),
	}
	...
	impl<T: Config> Pallet<T> {
		...
		#[pallet::weight(10000)]
		pub fn submit_price_signed(origin: OriginFor<T>, number: u64, mill: Permill) -> DispatchResult {
			let who = ensure_signed(origin)?;
			log::info!("submit_price_signed: ({}, {}, {:?})", number, mill.deconstruct(), who);
			Self::append_or_replace_price(number, mill);

			Self::deposit_event(Event::NewPrice(Some(who), number, mill.deconstruct()));
			Ok(())
		}
	}
	...
	impl<T: Config> Pallet<T> {
		...
		fn append_or_replace_price(number: u64, mill: Permill) {
			Prices::<T>::mutate(|prices| {
				if prices.len() == NUM_VEC_LEN {
					let _ = prices.pop_front();
				}
				prices.push_back((number, mill));
			})
		}
		...
		fn fetch_price_info() -> Result<(), Error<T>> {
			...
			let price: (u64, Permill) = Self::fetch_dot_price().map_err(|_| <Error<T>>::FetchPriceError)?;
			// 保证所有链上所有操作皆为签名后操作保证链上数据安全性
			let result = signer.send_signed_transaction(|_acct|
				// This is the on-chain function
				Call::submit_price_signed(price.0, price.1));

			if let Some((account, res)) = result {
				if res.is_err() {
					log::error!("fail to update price: {:?}", account.id);
					return Err(<Error<T>>::OffchainSignedTxError);
				}
				log::info!("send price successful");
		...
		fn fetch_dot_price() -> Result<(u64, Permill), Error<T>> {
			// Do http request
			log::info!("sending request to: {}", HTTP_REMOTE_DOT_PRICE_REQUEST);
			let request = rt_offchain::http::Request::get(HTTP_REMOTE_DOT_PRICE_REQUEST);
			...
			let price: &str = asset["data"]["priceUsd"].as_str().unwrap();
			log::info!("price is {:?}", price);
			let price_vec: Vec<&str> = price.split(".").collect();
			let number = price_vec[0].parse::<u64>().map_err(|_| <Error<T>>::HttpFetchingError)?;
			let mill = price_vec[1].get(..5).unwrap().parse::<u32>().map_err(|_| <Error<T>>::HttpFetchingError)?;
			log::info!("number is {}, mill is {}", number, mill);
			return Ok((number, Permill::from_parts(mill)));
		}
